...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 初始化数据
            async getData() {
                const loginLogs = await this.$http.get(baseURL_.sysUrl+'/sys/loginLogs/list',{ 
                    params: {'page':this.page,'pageSize':this.pageSize,'name':this.name}
                    });
                if(loginLogs.data.code==200){
                  this.tableData=loginLogs.data.data.records;
                  this.total=loginLogs.data.data.total;
                  this.page=loginLogs.data.data.current;
                }
            },
